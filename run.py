...
def main():
    ...
    if len(cur_ckpts) > 0 and not configs.only_eval:
        ...
        ckpt_load_dir = os.path.join(configs.save_path, configs.name, latest_checkpoint)

        configs.load_model_path = ckpt_load_dir
    ...
    if configs.coconut:
        if configs.state_combination_method == 'add':
            model = Coconut(model, latent_id, start_id, end_id, tokenizer.eos_token_id)
        elif configs.shared_para:
            model = Coconut_shared(model, latent_id, start_id, end_id, tokenizer.eos_token_id,
                                   state_combination_method=configs.state_combination_method,   # specify the combination method
                                    combination_use_gating=configs.combination_use_gating)
        else:
            model = Coconut_no_shared(model, latent_id, start_id, end_id, tokenizer.eos_token_id,
                                   state_combination_method=configs.state_combination_method,   # specify the combination method
                                    combination_use_gating=configs.combination_use_gating)
    ...
    if configs.reset_optimizer:
        ...
        optimizer = optim.AdamW(
            [p for p in parallel_model.parameters() if p.requires_grad],
            lr=configs.lr,
    ...
    for epoch in range(configs.resume, configs.num_epochs):
        ...
        with torch.no_grad():
            ...
            for idx, batch in enumerate(valid_gen_dataloader):
                ...
                # synced_gpus=True in DDP mode, as we need to keep # forward pass the same on each device
